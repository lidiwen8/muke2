...
<head>
    ...
    <c:if test="${sessionScope.user != null||sessionScope.admin!=null}">
        <script type="text/javascript" src="http://love.lidiwen.club/common.js"></script>
        <script type="text/javascript">
            document.write("<scr" + "ipt src=\"http://www.lidiwen.club:8083/ckeditor/ckeditor.js\" language=\"JavaScript\" charset=\"gb2312\"></sc" + "ript>")
        </script>
    </c:if>
    <title>爱之家网站</title>
    <script type="text/javascript">
    ...
    <script type="text/javascript">
            ...
            $.get("messageServlet",
                ...
                function (data) {
                    ...
                    if (data.res == -1) {
                        ...
                        if ($("#userid").val() != message.userid) {
                            msg.find(".showWindow").attr("onclick", "showWindow(" + -1 + "," + message.msgid + ")");
                            msg.find(".reportButton").attr("style", "display:block;");
                        }
                        </c:if>
                ...
                function (data) {
                    if (data.res == -1) {
                        ...
                        if (parseInt(data.reply.totalPage) > parseInt(pageNum)) {
                            $("#loadmore").html("加载更多...");
                            // $("#loadmore").attr("onclick", "javascript:getReply();");
        ...
        window.onscroll = function () {
            ...
            if (scrollTop + clientHeight == scrollHeight) {//当距离底部超过300px时，显示图片
                document.getElementById('backDown').style.display = "";
            } else { //距离顶部小于300px，隐藏图片
                document.getElementById('backDown').style.display = "";
            }
        ...
        function getMore() {
            if (confirm("请先登录！登陆后查看更多内容") != "0") {
                window.location.href = "<%=basePath%>login.jsp?msgid=" + msgId;
            }
        ...
        function createWebSocket(wsUrl) {
            try {
                ...
                websocket.onmessage = function (event) {
                    ...
                    if (event.data.split(",")[0] == "ReplyAdd" + msgId) {
                        // alert(event.data.split(",")[1]);
                        // location.reload();
                        var userid = event.data.split(",")[2];
                        if ($("#userid").val() == null || ($("#userid").val() != null && $("#userid").val() != userid)) {
                            var messageReply = confirm("该帖子有新消息回复了，是否查看最新回复w(゜Д゜)w");
                    ...
                    if (event.data.split(",")[0] == "ReplyUpdate" + msgId || event.data.split(",")[0] == "ReplyDelete" + msgId || event.data.split(",")[0] == "ReplyDeleteByAuther" + msgId||event.data.split(",")[0]=="ReplyDeleteAuther"+msgId||event.data.split(",")[0]=="ReplyUpdateAuther"+msgId) {
                        // getReply(-1);更新和删除要得到该条回复在第几页第几个记录，然后获取到后重新加载
                        //event.data.split(",")[1]是第几页，event.data.split(",")[2]是第几条
                        var userid = event.data.split(",")[3];
                        if ($("#userid").val() == null || ($("#userid").val() != null && $("#userid").val() != userid && (event.data.split(",")[0] != "ReplyDeleteByAuther" + msgId||event.data.split(",")[0]=="ReplyDeleteAuther"+msgId||event.data.split(",")[0]=="ReplyUpdateAuther"+msgId))) {
                        ...
                        if (($("#userid").val() != null && $("#userid").val() == userid) || (event.data.split(",")[0] != "ReplyDeleteByAuther" + msgId||event.data.split(",")[0]=="ReplyDeleteAuther"+msgId||event.data.split(",")[0]=="ReplyUpdateAuther"+msgId)) {
                        ...
                        if ($("#userid").val() == null || $("#userid").val() == '') {
                            var messageReplyVary = confirm("最新通知：该帖子之前的回复消息被点赞了(在第" + event.data.split(",")[1] + "页，第" + event.data.split(",")[2] + "条回复)，是否查看最新点赞w(゜Д゜)w");
                            if (messageReplyVary == true) {
                                location.reload();
                            }
                        }
                        if (($("#userid").val() != null && $("#userid").val() == userid)) {
                            var messageReplyZan = confirm("最新通知：你之前对该帖子的回复消息被其它用户点赞了(在第" + event.data.split(",")[1] + "页，第" + event.data.split(",")[2] + "条回复)，是否查看最新点赞w(゜Д゜)w");
                            if (messageReplyZan == true) {
                                location.reload();
                            }
                        }
                        if ($("#userid").val() != null && $("#userid").val() != userid) {
                        ...
                        if (($("#userid").val() != null && $("#userid").val() == userid)) {
                            var messagezan = confirm("最新通知：你的该帖子被其它用户点赞了，是否查看最新点赞w(゜Д゜)w");
                            if (messagezan == true) {
                                location.reload();
                            }
                        }
                    ...
                    if (event.data == "add" + msgId) {
                        // var userid=event.data.split(",")[1];
                        <%--<c:if test="${sessionScope.user== null||sessionScope.user.userid!=userid}">--%>
                        var messageVary = confirm("该帖子主题消息有改动，是否选择查看最新w(゜Д゜)w");
                        if (messageVary == true) {
                            location.reload();
                        }
                        <%--</c:if>--%>
...
<div class="container" id="msgList">
    ...
    <div class="row reply template">
        ...
        <div class="col-sm-2 col-xs-2">
            ...
            <div><a href='' class="userimg1"><img style="border-radius:50%" width="55px" height="55px" class="userimg"
                                                  name="userimg" id="userimg" style="vertical-align:middle"></a></div>
            <div class="sex"></div>
            <div class="city"></div>
        </div>
        <div class="col-sm-10 col-xs-10">
            <div class="emoji">
                ...
                <div>
                    <button class="btn btn-white btn-xs zan" style="height: 22px;float: right;">
                    ...
                    <c:if test="${sessionScope.user == null}">
                        <p style="display: block">
                            <a href="javascript:;" id="mgc_post_15215922" onmouseover="showMenu(this.id)"
                               class="showmenu" style="display: none;"></a>
                            <a href="javascript:;" data-toggle="modal"
                               onclick="alert('请先登录！')">举报</a>
                        </p>
                    </c:if>
                    <c:if test="${sessionScope.user != null}">
                        <p style="display: none" class="reportButton">
                            <a href="javascript:;" id="mgc_post" onmouseover="showMenu(this.id)" class="showmenu"
                               style="display: none;"></a>
                            <a href="javascript:;" class="showWindow" data-toggle="modal"
                               data-target="#report">举报</a>
                        </p>
                    </c:if>
                </div>
...
<div class="container">
    <div class="row">
            ...
            <c:choose>
                <c:when test="${sessionScope.user!= null ||sessionScope.admin!=null }">
                    <button id="loadmore" type="button" class="btn btn-default btn-lg btn-block"
                            onclick="getReply()" disabled="disabled">加载更多...
                    </button>
                </c:when>
                <c:otherwise>
        ...
        <script>window._bd_share_config = {
            ...
            "image": {
                "viewList": ["weixin", "sqq", "qzone", "tsina", "tqq", "renren"],
                "viewText": "分享到：",
                "viewSize": "16"
            },
            "selectShare": {
                "bdContainerClass": null,
                "bdSelectMiniList": ["weixin", "sqq", "qzone", "tsina", "tqq", "renren"]
            }
        };
...
<c:if test="${sessionScope.user!= null ||sessionScope.admin!=null }">
                    ...
                    <form id="replyform2">
                        <div class="form-group">
                            ...
                            <script type="text/javascript">
                                ...
                                <%--<ckeditor:replace replace="replycontent2" basePath="ckeditor/" />--%>
                        </div>
                        <div class="text-right">
    ...
    <div class="modal fade" id="report" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content modalcenter">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true" title="关闭">&times;
                    </button>
                    <h4 class="modal-title" id="reportLabel">举报</h4>
                    <table cellpadding="0" cellspacing="0" class="fwin">
                        <tbody>
                        <tr>
                            <td class="t_l"></td>
                            <td class="t_c" style="cursor:move"
                                onmousedown="dragMenu($('fwin_miscreport15215922'), event, 1)"
                                ondblclick="hideWindow('miscreport15215922')"></td>
                            <td class="t_r"></td>
                        </tr>
                        <tr>
                            <td class="m_l" style="cursor:move"
                                onmousedown="dragMenu($('fwin_miscreport15215922'), event, 1)"
                                ondblclick="hideWindow('miscreport15215922')" )
                            "="">&nbsp;&nbsp;</td>
                            <td class="m_c" id="fwin_content_miscreport15215922" fwin="miscreport15215922" style="">
                                <h3 class="flb" id="fctrl_miscreport15215922" style="cursor: move;">
                                </h3>
                            ...
                            <td class="m_r" style="cursor:move"
                                onmousedown="dragMenu($('fwin_miscreport15215922'), event, 1)"
                                ondblclick="hideWindow('miscreport15215922')">
                        </tr>
                        <tr>
                            <td class="b_l"></td>
                            <td class="b_c" style="cursor:move"
                                onmousedown="dragMenu($('fwin_miscreport15215922'), event, 1)"
                                ondblclick="hideWindow('miscreport15215922')"></td>
                            <td class="b_r">
                            </td>
                ...
                <div class="modal-body">
                    <form style="width:28em" method="post" autocomplete="off" id="form_report"
                          name="form_miscreport15215922" action="misc.php?mod=report"
                          onsubmit="if(!$('report_message').value) return false;ajaxpost(this.id, 'form_miscreport15215922');"
                          fwin="miscreport15215922">
                        <div class="reason_slct c" id="return_miscreport15215922" fwin="miscreport15215922">
                            ...
                            <p>请点击举报理由</p>
                            <p class="mtn mbn" id="report_reasons" fwin="miscreport15215922"><label
                                    style="cursor: pointer"><input type="radio" name="report_select" class="pr"
                                                                   onclick="$('#report_other').css('display','none');$('#report_msg').css('display','none')"
                                                                   value="广告垃圾"> 广告垃圾</label><br><label
                                    style="cursor: pointer"><input type="radio" name="report_select" class="pr"
                                                                   onclick="$('#report_other').css('display','none');$('#report_msg').css('display','none')"
                                                                   value="违规内容"> 违规内容</label><br><label
                                    style="cursor: pointer"><input type="radio" name="report_select" class="pr"
                                                                   onclick="$('#report_other').css('display','none');$('#report_msg').css('display','none')"
                                                                   value="恶意灌水"> 恶意灌水</label><br><label
                                    style="cursor: pointer"><input type="radio" name="report_select" class="pr"
                                                                   onclick="$('#report_other').css('display','none');$('#report_msg').css('display','none')"
                                                                   value="重复发帖"> 重复发帖</label><br><label
                                    style="cursor: pointer"><input type="radio" name="report_select" class="pr"
                                                                   onclick="openReportMsg()" value="其他"> 其他</label><br>
                            </p>
                            <div id="report_other" style="display:none;" fwin="miscreport15215922">
                                <textarea placeholder="请填写举报内容"
                                          style="margin: 0px; width: 557px; height: 101px;resize:vertical;"
                                          id="report_message" name="message" class="reasonarea pt mtn xg1"
                                          onfocus="this.innerHTML='';this.focus=null;this.className='reasonarea pt mtn'"
                                          onkeydown="ctrlEnter(event, 'reportsubmit', 1);" rows="4"
                                          fwin="miscreport15215922">请填写举报内容</textarea>
                            </div>
                        </div>
                        <p class="o pns">
                            <span id="report_msg" style="display:none" fwin="miscreport15215922"><span
                                    class="z">还可输入 <strong id="checklen"
                                                           fwin="miscreport15215922">200</strong> 个字符</span></span>
                        </p>
...
<div id="backUp" title="返回顶部"
     style="z-index: 9999; position: fixed ! important; right: 5px; bottom: 70px;cursor: pointer">
    <img src="upload/top.png"/>
</div>
<div id="backDown" title="返回底部"
     style="z-index: 9999; position: fixed ! important; right: 5px; bottom: 30px;cursor: pointer">
