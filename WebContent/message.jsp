	...
	function getReply(){
		...
		function(data){
			...
			reply.find(".order").text(number);//楼层
                ...
                if(this.sex=='女')
                {
                   reply.find(".author").html(this.realname+'<img width="50" class="emoji" style="vertical-align:middle" src="${user.user_img}">');
                    // images/nu.png
                    // images/nan.png
                }else{
                    reply.find(".author").html(this.realname+'<img width="50" class="emoji" style="vertical-align:middle" src="${user.user_img}">');
...
//			reply.find(".author").text(this.realname);//真实姓名
			...
			reply.find(".city").text(this.city);//城市this.replycontents
...
//        var len=CKEDITOR.instances.replycontent.document.getBody().getText().trim().length;||len==0
                if($.trim(replycontents).length==0){
                 alert('输入不能为空,回帖失败!');
                 return;
               }if(replycontents=='黄晓琪'||replycontents=='傻逼'||replycontents=='傻'||replycontents=='操'||replycontents=='日你老母'||replycontents=='骚货'){
			alert('对不起，由于你的输入中包含敏感词汇,回帖失败!');
			return;
		}
		else{
  ...
  toTop.onclick = function(){ //点击图片时触发的点击事件
   ...
   function zan2(luntanid,zan) {
//当前赞的人
        var loginid=$(".loginid").val();
        $.ajax({
            url:"messageServlet",//跳转界面
            type : "POST",
            async : "true",
            data : {"action" : "zan", "pageNum": pageNum},
            dataType:"json",
            success:function(data){
                alert(data.name);
                $(zan).find('.zancount').text(data.count);//找到当前点击的这个赞让其为查到的赞数
                $(zan).attr("disabled","disabled");//让当前赞不可再点击
                $(zan).removeAttr("onclick");//移除onclick事件
                $(zan).parent().next().find('ul').append('<li>'+data.name+'</li>');//显示赞的人
            }
        })
    };
</script>
			...
			<div class="col-sm-10 col-xs-10">
				<div class="emoji">
					<div class="msgcontent"></div>
                                        <div><a class="btn btn-white btn-xs zan btns" onclick="zan2(${i.id},this)"style="height: 22px;float: right;" >
						<i class="fa fa-thumbs-up"></i> 赞:<img alt="" src="http://www.lidiwen.club/muke_Web/ckeditor/plugins/smiley/images/79.gif" style="height:17px; width:17px" title="" />&nbsp;&nbsp;
						<span class="zancount">${i.zanCount==0?"":i.zanCount}</span>
					</a></div>
