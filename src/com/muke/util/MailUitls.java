...
public class MailUitls {
	...
	public void sendMail(String mail, String code) {
		...
		try {
			...
			message.setSubject("爱之家科技有限公司");// 设定主题

			message.setSentDate(new Date());// 设定发送时间
			// 设置邮件正文:
			message.setContent(
					"<h1>爱之家网站答疑系统官方激活邮件!点下面链接完成激活操作!</h1><h3><a href='http://"
							+ addr
							+ ":8080/ssm_OAManager/view/user/active.action?code="
							+ code
							+ "'>http://"
							+ addr
							+ ":8080/ssm_OAManager/view/user/active.action?code="
	...
	public void sendMailFindUser(String mail) {
		...
		try {
			...
			// 设置邮件正文:
			message.setContent(
					"<h1>爱之家网站答疑系统官方重置密码邮件!点下面链接完成密码重置操作!</h1><h3><a href='http://"
